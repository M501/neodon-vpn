...
**F. Пресеты правил (7 актуальных; 4 V1-наследия удалены 2026-09-08):** каждый пресет: apply → валидность конфига (`sing-box check`), изменение `.profile`, проверка первых правил по логам маршрута (karl: direct vs proxy), возврат в `default`. Минимум — smoke на каждый из 7 + e2e на 3 базовых.
