...
## 2026-09-16 (54) — Релиз v0.1.6: панель не врёт, трей возвращается, таймер честный

- https://github.com/M501/neodon-vpn/releases/tag/v0.1.6
- Игровой режим НЕ останавливал VPN: 0 vpn_down за день, NRestarts=0,
  exit-IP жив, трафик идёт при закрытой панели. Врал показ: ремаунт
  стартовал с Off до опроса. Фикс: первый кадр Syncing…, опрос одним
  рейсом (Promise.all), удалена мёртвая строка вне quiet-окна.
- Десктоп после возврата: процесса GUI не было вовсе (сессия прибила,
  автостарта не было) — флага и не могло быть. Фикс: автостарт
  индикатора (только показ, VPN не трогает) + таймер сеется от старта
  сервиса (seed_epoch), а не от запуска окна.
- QA: pytest+backend+harness на хосте, triple-pass, soak 60с без панели.

## 2026-09-12 (53) — Релиз v0.1.5: фантомный довызов тумблера в QAM

- https://github.com/M501/neodon-vpn/releases/tag/v0.1.5
- Баг: вкл→выкл за 1-2с — переподсвет. Доказано: vpn_down→vpn_up за 2с
  без нажатия (Steam перевызывает onChange при флипе checked опросом).
- Фикс: seenRef/wantRef + quiet 8с; настоящее переключение — ровно раз.
- GitHub прибран: README-витрина (скачать→распаковать→двойной клик),
  описание + топики репозитория.
- QA triple-pass зелёный.

## 2026-09-11 (52) — Релиз v0.1.4: честная кнопка + установка двойным кликом

- https://github.com/M501/neodon-vpn/releases/tag/v0.1.4
- Кнопка питания решает по намерению (_target/pending), а не по протухшему
  UI: вкл→выкл за 1с queue'ит OFF, а не второй ON. Bounce-guard 5с→1.5с.
  По пути поймана собственная инверсия в тернарнике (3 теста упали, починено).
- test_power_intent_flips_while_busy; 56/56 хост.
- Установка для бабушки: «Install Neodon VPN.desktop» в корне тарбола
  (двойной клик → окно терминала с прогрессом [1/7]–[7/7] → пароль один
  раз → NEXT-инструкция). Терминал руками не нужен. README с 3 шагами.
- QA triple-pass зелёный.

## 2026-09-11 (51) — Трей: флаг застревал после OFF

- Баг: при OFF `setIcon` не вызывался вообще (ветка theme только для
  CONNECTED/LOCKED/FAILED) — оставался флаг последнего сервера.
- Фикс: дефолтный щит запоминается при setup, OFF и остальные —
  явный restore (theme-иконка если есть, иначе щит).
- test_tray_off_restores_default_icon; 55/55 хост; файл в прод-пути.

## 2026-09-11 (50) — Релиз v0.1.3: кламп окна в тарболе

- https://github.com/M501/neodon-vpn/releases/tag/v0.1.3
- Внутри: _clamp_to_screen (окно чинится переоткрытием) + всё из 0.1.2.
- QA triple-pass зелёный.

## 2026-09-11 (49) — Окно под доком: кламп в availableGeometry

- Жалоба: уголок ресайза уехал под нижние иконки, мышью не достать.
- Фикс: `_clamp_to_screen()` после restore — окно всегда внутри
  доступной геометрии (мимо панелей/доков): размер + позиция.
  Переоткрытие чинит любое чужое растягивание.
- Срочный выход без кода: Alt+правая кнопка по окну — ресайз из любой
  точки; Alt+левая — двигать.
- test_clamp_to_screen; 54/54 хост; файл в прод-пути (в следующий релиз).
