...
- Состав: backend-скрипты, GUI, юниты, desktop+иконка+лаунчер neodon-gui
  (без хардкода /home), sudoers-шаблон, профили (без секретов, скан),
  флаги, примеры конфигов с плейсхолдером, Decky-плагин со собранным
  dist, install.sh + verify.sh.
- Проверено: stage из репо + secret-scan + install --dry-run на хосте.
- Windows→uploads flaky (2 отката), прошло с ретрая. Паролей не требует
  кроме одного sudo при установке; секреты не шипятся (плейсхолдеры).

## 2026-09-10 (45) — «FAILED без попыток»: это был neodon-boot, не состояние

- Жалоба: свежий вход → FAILED, хотя никто не нажимал. Разбор: `off`
  честно пишет desired=off; FAILED выдаётся только при desired != off
  + мёртвый сервис. В том заходе desired=smart записал neodon-boot
  при загрузке (18:21:01) — та же причина, уже убита в (44).
- Живой пруф: desired=off, actual OFF, маркера transition нет.
- Семантика зафиксирована: OFF = никто не нажимал; FAILED = «был включён,
  сейчас лежит — нажми для переподключения». TRANS_MARKER lifecycle чист.

## 2026-09-10 (44) — Автоконнект при загрузке: виноват neodon-boot.service

- Жалоба «после рестарта сам подключился»: sing-box встал через 14с
  после boot (.mode записан в 18:21:01) при disabled-юнитах.
- Корень: пропущенный при аудите `neodon-boot.service` («Neodon VPN
  autostart», `ExecStart=...toggle.sh smart`, enabled) + 3-й юнит
  `sing-box-proxy.service` (enabled). Все 4 теперь disabled, сессия жива.
- install.sh: units ставятся + сразу disable (коммент-доктрина поправлена);
  verify.sh чек расширен на все 4 — VERIFY OK.
- Хук на вход в гейм-мод не понадобился: с мёртвым boot-автостартом
  игровой режим стартует с OFF по построению (стартовать нечему).

## 2026-09-10 (43) — Spec 029: только ручное включение, select не коннектит

- Найден и вырезан авоконнект десктопа при выборе сервера
  (`start_after = state != CONNECTED` → всегда False). Вкл/выкл теперь
  только с главной кнопки в обоих концах; выбор лишь stage'ит.
- Проверено чтением: десктоп при старте не зовёт toggle; decky и так
  select-only; set_server при простое сервисов не стартует.
- Два VPN невозможны: один unit/TUN/MODE_FILE/flock; живьём pgrep -c = 1.
- Переход десктоп→игра: состояние общее, второй VPN не заводится;
  авто-гашения нет сознательно (приказ «только я нажимаю»).
- test_server_select_never_connects; 53/53 хост; файл в прод-пути.

## 2026-09-10 (42) — VPN никогда сам: автозапуск выключен, страж в verify

- Аудит: оба user-юнита были enabled (включались при входе) — выключены
  (`disable`, без `--now`: текущая сессия и VPN тронуты не были).
- Таймеров нет, десктоп-автостарта нет, watchdog действует только при
  desired != off, плагин при входе лишь читает статус (vpn_up только
  с тумблера). Релизный install.sh и так disable'ил — живые юниты
  включились когда-то вручную.
- verify.sh: новый жёсткий чек `no autostart (default OFF)` — VERIFY OK.

## 2026-09-10 (41) — Замер под нагрузкой + формат часов + имена правил verbatim

- Нагрузка (CONNECTED, smart): sing-box 3 тика/5с (~0.6% ядра), 0.3% MEM;
  бэкенд 0.0% CPU. Система 30.3W vs 21.3W — оба замера на зарядке, дельта
  не атрибутируется VPN (зарядный ток + активность). Сигнал — по процессам.
- Часы: `p(h)` — всегда 8 символов `00:14:22`, как десктоп.
- Имена правил возвращены дословно (десктоп PRESETS + PRESET_NAMES);
  страж `preset-names-verbatim` падает при любом переименовании.
- Traceback в журнале при рестарте — гонка самого лоадера
  (PluginStopped, задело и Framegen), не наш код; новый инстанс чист.
